...
if ($Mode -eq "local") {
    ...
    $lines.Add('param ([Parameter(Position=0)][string]$Action = "help", [Parameter(ValueFromRemainingArguments=$true)][string[]]$Options)')
    ...
    $lines.Add('    Write-Host "  Usage: orionrouter [start [--silent]|stop|logs|help]" -ForegroundColor Yellow')
    $lines.Add('    Write-Host ""')
    $lines.Add('    Write-Host "  Commands:"')
    $lines.Add('    Write-Host "    start [--silent]  Starts the server in the background (silent mode optional)"')
    ...
    $lines.Add('        return')
    $lines.Add('    }')
    $lines.Add('    Write-Host "Orion Router started. Opening logs in a new window..." -ForegroundColor Green')
    $lines.Add('    Start-Process powershell -ArgumentList "-NoExit", "-Command", "orionrouter logs"')
    ...
    $lines.Add('    Write-Host "  --------------------------------" -ForegroundColor DarkGray')
    $lines.Add('    Write-Host "  Usage: orionrouter [start [--logs]|stop|logs|help]" -ForegroundColor Yellow')
    $lines.Add('    Write-Host ""')
    $lines.Add('    Write-Host "  Commands:"')
    $lines.Add('    Write-Host "    start [--logs]    Starts container in the background (with live logs)"')
    ...
    $lines.Add('    if ($Options -contains "--logs" -or $Options -contains "-logs" -or $Options -contains "logs") {')
    $lines.Add('        Write-Host "Streaming live logs... (Ctrl+C to exit)" -ForegroundColor Magenta')
    $lines.Add('        docker compose -f $ComposeFile -p orion-router logs -f')
    $lines.Add('    } else {')
    $lines.Add('        Write-Host "Orion Router started on Docker." -ForegroundColor Green')
    $lines.Add('        Write-Host "To view logs, run: orionrouter logs" -ForegroundColor Cyan')
    $lines.Add('    }')
    $lines.Add('} elseif ($Action -eq "stop") {')
