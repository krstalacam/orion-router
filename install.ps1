...
if ($Mode -eq "local") {
    ...
    $lines.Add('param ([Parameter(Position=0)][string]$Action = "help", [Parameter(Position=1)][string]$Option = "")')
    ...
    $lines.Add('    if ($Option -eq "-silent" -or $Option -eq "--silent" -or $Option -eq "silent") {')
    $lines.Add('        Write-Host "Orion Router started in the background (Silent)." -ForegroundColor Green')
    $lines.Add('        return')
    $lines.Add('    }')
    $lines.Add('    Write-Host "Streaming live logs... (Will close automatically when stopped)" -ForegroundColor Magenta')
    $lines.Add('    $timeout = 0')
    $lines.Add('    while (!(Test-Path $LogFile) -and $timeout -lt 50) {')
    $lines.Add('        Start-Sleep -Milliseconds 100')
    $lines.Add('        $timeout++')
    $lines.Add('    }')
    $lines.Add('    if (Test-Path $LogFile) {')
    $lines.Add('        $reader = New-Object System.IO.StreamReader([System.IO.File]::Open($LogFile, [System.IO.FileMode]::Open, [System.IO.FileAccess]::Read, [System.IO.FileShare]::ReadWrite))')
    $lines.Add('        try {')
    $lines.Add('            while ($null -ne (Get-Process -Id $p.Id -ErrorAction SilentlyContinue)) {')
    $lines.Add('                $line = $reader.ReadLine()')
    $lines.Add('                if ($null -ne $line) {')
    $lines.Add('                    Write-Host $line')
    $lines.Add('                } else {')
    $lines.Add('                    Start-Sleep -Milliseconds 100')
    $lines.Add('                }')
    $lines.Add('            }')
    $lines.Add('            while ($null -ne ($line = $reader.ReadLine())) {')
    $lines.Add('                Write-Host $line')
    $lines.Add('            }')
    $lines.Add('        } finally {')
    $lines.Add('            $reader.Close()')
    $lines.Add('        }')
    $lines.Add('    }')
    ...
    $lines.Add('    if ($Option -eq "-silent" -or $Option -eq "--silent" -or $Option -eq "silent") {')
    $lines.Add('        Write-Host "Orion Router started on Docker (Silent)." -ForegroundColor Green')
    $lines.Add('        return')
    $lines.Add('    }')
    $lines.Add('    Write-Host "Streaming live logs... (Ctrl+C to exit)" -ForegroundColor Magenta')
