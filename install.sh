...
if [ "$ACTION" = "help" ] || [ -z "$ACTION" ]; then
    ...
    echo "  --------------------------------"
    echo "  Usage: orionrouter <command> [options]"
    echo ""
    echo "  Commands:"
    echo "    start [--silent]  Starts the server in the background (silent mode optional)"
...
if [ "$ACTION" = "help" ] || [ -z "$ACTION" ]; then
    ...
    echo ""
    echo "  Commands:"
    echo "    start [--logs]    Starts container in the background (with live logs)"
